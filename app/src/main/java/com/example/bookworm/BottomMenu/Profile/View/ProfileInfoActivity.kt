...
import com.example.bookworm.Extension.Follow.Interfaces.PagerInterface.PageAdapter
...
//    var followCounter = followCounter(null, -1)
    ...
    suspend fun setUI(user: UserInfo) {
        ...
        binding.tvFollow.setOnClickListener {
            if (binding.tvFollow.isSelected) {
                ...
                binding.tvFollow.text = "팔로우"
                lifecycleScope.launch {
                    setFollowerCnt(async { fv.follow(user, false) }.await().followerCounts.toLong())
                }
            } else {
                binding.tvFollow.isSelected = true
                binding.tvFollow.text = "팔로잉"
                lifecycleScope.launch{
                    setFollowerCnt(async { fv.follow(user, true) }.await().followerCounts.toLong())
                }
            }

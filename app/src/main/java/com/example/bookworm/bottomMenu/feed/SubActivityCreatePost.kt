...
class SubActivityCreatePost : AppCompatActivity() {
    ...
    private val feedData by lazy {
        Feed().apply {
            ...
            intent.apply {
                isUserPost = true
                creatorInfo =
                    if (hasExtra(mainUserKeyword)) getParcelableExtra(mainUserKeyword)!! else UserInfo()
                    ...
                    "back" -> {
                        ...
                        btnYes.setOnClickListener {
                            if (feedData.book == Book() || feedData.feedText == "") {
                                Toast.makeText(
                                    context,
                                    "내용을 입력한 후 완료를 선택해 주세요.",
                                    Toast.LENGTH_SHORT
                                ).show()
                            } else if (feedData.feedText!!.length < 10) {
                                Toast.makeText(context, "리뷰를 10자 이상 입력해주세요.", Toast.LENGTH_SHORT)
                                    .show()
                            } else {
                                processUpload()
                            }
    ...
    private fun processUpload() {
        ...
        feedData.feedID = System.currentTimeMillis()
            .toString() + "_" + feedData.userToken //현재 시각 + 사용자 토큰을 FeedID로 설정

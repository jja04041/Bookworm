...
class customMenuPopup(val context: Context, anchor: View) : PopupMenu(context, anchor) {
    ...
    inner class menuItemClickListener(val data: Any, val context: Context)
        ...
        //피드 관련 메뉴 변수
        private val FEED_DELETE = 0 // 삭제
        private val FEED_MODIFY = 1 // 수정
        ...
        fun modifyData(item: Any) {
            if (item is Feed) {
                ...
                var intent = Intent(context as FeedActivity, subActivity_Feed_Modify::class.java)
        ...
        fun createAlertBuilder(code: Int) =
                AlertDialog.Builder(context)
                        ...
                        .setPositiveButton("네") { dialog: DialogInterface, which: Int ->
                            ...
                            if (code == FEED_DELETE) {
                                //뷰모델 생성
                                vm = ViewModelProvider(if (context is FeedActivity) context
                                else context as subactivity_comment)
                                        .get(FeedViewModel::class.java)

                                //뷰모델을 이용하여 서버에서 피드 데이터 삭제 진행
                                (vm as FeedViewModel).deleteFeed(data as Feed)
//                                (vm as FeedViewModel).onViewEvent(FeedViewModel.FeedViewEvents.Remove(data as Feed))
                                //어댑터 Refresh
                                //삭제된 내용을 현재 액티비티에 반영해야함.
                                if (context is subactivity_comment) context.finish()
                                ((context as FeedActivity).supportFragmentManager
                                        .findFragmentByTag("0") as fragmentFeed)
                                        .pageRefresh() //새로 데이터를 로드
                            }

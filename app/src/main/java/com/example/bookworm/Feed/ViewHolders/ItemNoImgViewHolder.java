...
//피드에 이미지가 없는 경우 이 뷰홀더를 사용한다.
// => 리사이클러뷰는 뷰를 재사용하다보니, 이미지가 있는 기존 뷰를 재활용 하여,
// 이미지가 없는 피드 데이터를 집어 넣게 되면, 이미지뷰가 비활성화가 안된 채 나타나는 오류 발생
public class ItemNoImgViewHolder extends RecyclerView.ViewHolder {
